...
const CustomDrawer: React.FC<DrawerContentComponentProps> = ({ navigation }) => {
  ...
  return (
    <View style={{ backgroundColor: colors.primaryLight }} className="flex-1 pt-16 px-5">
      {/* Header */}
      <View 
        className="relative items-center mb-10 border-b " 
      ...
      >
        ...
        <Image
          source={
            business?.logo ? { uri: business.logo } : require("../assets/logo.png")
          }
          className="size-40 rounded-full mb-2"
          onError={() => console.log("Image load error, using default")}
        />
