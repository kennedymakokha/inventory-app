...
export const getProductSalesReport = async ({
    ...
    userId = null,      // only needed if userRole === 'sales'
    filterId = 1,
    page = 1,
    pageSize = 20,
}) => {
    ...
    switch (filterId) {
        case 1: // Today
            ...
            break;
        case 2: // Last week
            dateCondition = "s.created_at >= DATE('now', '-7 days')";
            break;
        case 3: // Last month
            dateCondition = "s.created_at >= DATE('now', '-1 month')";
            break;
        case 4: // Last 3 months
            dateCondition = "s.created_at >= DATE('now', '-3 months')";
            break;
        default:
            dateCondition = "1=1";
    }
    ...
    let roleCondition = '';
    if (userRole === 'sales' && userId) {
        roleCondition = `AND s.createdBy = '${userId}'`; // sales user sees only their own sales
    }
    // Admin sees all → roleCondition empty
