...
export default function InventoryScreen() {
    ...
    const AddProduct = async () => {
        ...
        try {
            ...
            let storedItems: any = await getProducts(db);
            setInventories(storedItems);
        ...
        <View className="flex-1 min-h-[300px] bg-secondary-900 px-5">
                    ...
                    renderItem={({ item }: any) => (
                        <View className={`flex-row justify-between ${item.synced === 0 ? "bg-primary-100" : "bg-primary-50"} p-4 rounded-lg shadow-md mt-2`}>
                            ...
                            <TouchableOpacity
                                ...
                                className={`${item.quantity < 20 ? "bg-red-600" : "bg-green-600"} dark:bg-secondary-800 p-2 flex items-center justify-center rounded`}>
            ...
            <View className="absolute bottom-5 left-5 gap-y-5 right-5 z-50 items-center">
                <View className="flex-row w-full  ">
                    {/* <Fab icon="upload-to-cloud" outline loading={false} title="+" handleclick={() => console.log("first")} /> */}
