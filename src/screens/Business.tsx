...
const BusinessProfileScreen = () => {
    ...
    const { business, setBusiness, isLoading } = useBusiness();
    ...
    // Save to **remote server first**, then update context
    const handleSave = async () => {
        try {
            await updateBusinessRemotely({
                ...data, headers: {
                    "x-source": "socket"
                }
            }).unwrap();

            setBusiness(data); // update context separately
            setIsEditing(false);
        } catch (err) {
...
const styles = StyleSheet.create({
    ...
    fieldCard: { borderWidth: 1, borderRadius: 12, padding: 14, marginBottom: 16 },
    ...
    button: { flexDirection: "row", justifyContent: "center", alignItems: "center", marginTop: 20, padding: 14, borderRadius: 12, gap: 8 },
